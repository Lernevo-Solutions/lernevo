...
function User() {
  ...
  // ✅ Added Admin Functionality for Forced Password Reset
  const handleHardResetPassword = async (user) => {
    const confirmed = window.confirm(
      `Are you sure you want to hard reset password for ${user.username}?\nPassword will be set to default "Lernevo@123" and they must change it on next login.`
    );
    if (!confirmed) return;
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <p style={styles.headerP}>Manage codes, activations and strict resets</p>
      </div>
